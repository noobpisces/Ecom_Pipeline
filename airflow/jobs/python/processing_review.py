...
def clean_reviews(input_path, output_path):
    try:
        ...
        # Đọc dữ liệu từ CSV với schema đã chỉ định
        logging.info(f"Reading data from {input_path}")
        df = spark.read.format("parquet").option("header", "true").schema(schema).load(input_path)

        # Tiến hành làm sạch dữ liệu
        logging.info("Cleaning data...")
        df_cleaned = df.select(
            col("review").alias("review"),
            col("sentiment").alias("sentiment")
        ).na.drop().distinct()  # Xóa null và bản sao
        
        # Lưu dữ liệu đã làm sạch vào Delta Lake trên MinIO
...
if __name__ == '__main__':
    try:
        ...
        if len(sys.argv) != 3:
            logging.error("Usage: spark-submit clean_reviews.py <input_path> <output_path>")
            sys.exit(1)
